...
impl Keyboard {
    ...
    pub fn process(&mut self) -> Option<char> {
        ...
        match sc {
            0xAA | 0xB6 => { self.shift = false; None } // shift released
            0x2A | 0x36 => { self.shift = true;  None } // shift pressed
            0x3A        => { self.caps = !self.caps; None } // caps
            0x01        => { serial_println!("ESC"); None }
            code if code < 0x80 => {
